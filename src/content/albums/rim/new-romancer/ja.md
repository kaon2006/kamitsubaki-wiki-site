---
locale: ja
translationKey: rim_album_new_romancer
title: "NEW ROMANCER"
romanizedTitle: "NEW ROMANCER"
artist: "理芽"
type: "スタジオアルバム"
description: "理芽の正式リリース『NEW ROMANCER』。全曲順を確認し、各楽曲ページへリンクしています。"
releaseDate: "2021-07-21"
label: "℗ KAMITSUBAKI RECORD"
trackCount: 14
duration: "49:15"
categoryTitle: "アルバム"
categorySubtitle: ALBUMS
categoryOrder: 1
itemOrder: 1626850800000
image: /images/albums/rim/new-romancer.jpg
officialLinks:
  - label: "公式ディスコグラフィー"
    href: https://kamitsubaki.jp/discography/rim/703/
  - label: "Apple Music Japan"
    href: https://music.apple.com/jp/album/new-romancer/1688521731?uo=4
tracks:
  -
    number: "1"
    title: "ピロウトーク"
    duration: "03:42"
    songId: "rim/originals/track-1687461821"
  -
    number: "2"
    title: "ラヴソング"
    duration: "03:44"
    songId: "rim/originals/track-1688521734"
  -
    number: "3"
    title: "さみしいひと"
    duration: "03:02"
    songId: "rim/originals/track-1688521735"
  -
    number: "4"
    title: "宿木"
    duration: "03:04"
    songId: "rim/originals/track-1688521736"
  -
    number: "5"
    title: "いたいよ"
    duration: "03:16"
    songId: "rim/originals/track-1688521738"
  -
    number: "6"
    title: "甘美な無法"
    duration: "04:13"
    songId: "rim/originals/track-1687465026"
  -
    number: "7"
    title: "食虫植物"
    duration: "02:38"
    songId: "rim/originals/track-1688521740"
  -
    number: "8"
    title: "胎児に月はキスをしない"
    duration: "02:28"
    songId: "rim/originals/track-1687474051"
  -
    number: "9"
    title: "NEUROMANCE"
    duration: "03:57"
    songId: "rim/originals/track-1688521743"
  -
    number: "10"
    title: "ユーエンミー"
    duration: "03:55"
    songId: "rim/originals/track-1688512933"
  -
    number: "11"
    title: "クライベイビー"
    duration: "03:00"
    songId: "rim/originals/track-1688347821"
  -
    number: "12"
    title: "魔的 (feat. KAF)"
    duration: "03:41"
    songId: "rim/collaborations/track-1688521748"
  -
    number: "13"
    title: "やさしくしないで"
    duration: "04:27"
    songId: "rim/originals/track-1688521749"
  -
    number: "14"
    title: "十九月"
    duration: "04:07"
    songId: "rim/originals/track-1687478446"
---

## 作品概要

『NEW ROMANCER』は本カタログに収録する正式リリースです。発売日、曲順、収録時間、参加アーティストは公式ディスコグラフィーとApple Music日本版を照合しています。

## トラックとバージョン

全14トラックを掲載。各トラックは個別の楽曲・バージョンページへリンクします。再収録は同一ページを再利用し、ライブ、リミックス、リアレンジ版は別ページとしています。

## 資料方針

ジャケットはApple Musicの最高原生解像度を優先し、フィジカル限定作品は公式商品画像を使用します。

## 出典

- [公式ディスコグラフィー](https://kamitsubaki.jp/discography/rim/703/)
- [Apple Music Japan](https://music.apple.com/jp/album/new-romancer/1688521731?uo=4)
- [VGMdb 理芽](https://vgmdb.net/artist/53598)
